...
namespace translate
{
    ...
    {
        return [u](std::optional<std::string> & value) { u.mutate(value); };
    }

    template <>
    auto make_writer<translate::StdioWriter>(translate::StdioWriter && w)
    {
        return [w](std::string const & value) { w.write(value); };
    }
} // namespace translate

int main()
{
    translate::translate(translate::make_reader(translate::StringReader{}),
